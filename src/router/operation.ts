...
const importYtoj = async (req: Request, res: Response) => {
  ...
  let sql: string = "insert ignore into lightering (type,add_time,voyage,container_no,bl_no,customs_container_type,iso,container_type,container_holder,is_import,extra_operation,trade_type,customs_seal_no,seal_no,cargo_type,cargo_name,load_port,target_port,unload_port,load_payer,total_weight,cargo_weight,volume,amount,cargo_owner,forwarder,remarks) values ?";
...
const generateShipFee = async (req: Request, res: Response) => {
  ...
  select_item.forEach((item) => {
    ...
    select_sql += `select id from container where containner_no = '${item.container_no}' and track_no = '${item.bl_no}' and seal_no = '${item.seal_no}' and city = '武汉';`;
...
const importJtoy = async (req: Request, res: Response) => {
  ...
  let sql: string = "insert ignore into lightering (type,add_time,voyage,bl_no,load_port,unload_port,target_port,total_weight,container_no,container_holder,extra_operation,container_type,customs_container_type,iso,is_import,empty_weight,trade_type,customs_seal_no,seal_no,cargo_type,cargo_name,unload_payer,transfer_type) values ?"
...
const fixContainerInfo = async (req: Request, res: Response) => {
  ...
  let modifyParams = [];
  if (type == "暂落") {
    modifySql = "UPDATE dispatch as a left join container as b on b.id = a.container_id SET b.ship_name = ?, b.customer = ?, b.track_no = ?, b.containner_no = ?, b.seal_no = ?, b.container_type = ?, b.door = ?, b.load_port = ?, b.unload_port = ?, b.temp_port = ?, b.arrive_time = ?, b.make_time = ?, a.car_no = ?, a.add_time = ?  WHERE a.id = ?;select a.type, a.container_id, a.car_no as dispatch_car_no, a.add_time, b.* from dispatch as a left join container as b on b.id = a.container_id where a.id = ?;";
    modifyParams = [ship_name,customer,track_no,containner_no,seal_no,container_type,door,load_port,unload_port,temp_port,arrive_time,make_time,car_no,make_time,dispatch_id,dispatch_id];
  } else {
    modifySql = "UPDATE dispatch as a left join container as b on b.id = a.container_id SET b.ship_name = ?, b.customer = ?, b.track_no = ?, b.containner_no = ?, b.seal_no = ?, b.container_type = ?, b.door = ?, b.load_port = ?, b.unload_port = ?, b.temp_port = ?, b.car_no = ?, b.arrive_time = ?, b.make_time = ?, a.car_no = ?, a.add_time = ?  WHERE a.id = ?;select a.type, a.container_id, a.car_no as dispatch_car_no, a.add_time, b.* from dispatch as a left join container as b on b.id = a.container_id where a.id = ?;";
    modifyParams = [ship_name,customer,track_no,containner_no,seal_no,container_type,door,load_port,unload_port,temp_port,car_no,arrive_time,make_time,car_no,make_time,dispatch_id,dispatch_id];
  }
...
const addContainerFee = async (req: Request, res: Response) => {
  ...
  sql += `insert into container_fee (container_id, type, fee_name, amount, remark) values ('${id}', '应付', '${fee_name}', '${fee}', '${remark}');`;
  sql += `insert into container_fee (container_id, type, fee_name, amount, remark) values ('${id}', '应收', '${fee_name}', '${fee}', '${remark}');`;
  connection.query(sql, async function (err, data) {
    if (err) {
      console.log(err);
    } else {
