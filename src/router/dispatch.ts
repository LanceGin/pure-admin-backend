...
const importDispatchList = async (req: Request, res: Response) => {
  ...
  let sql: string = "select a.id as dispatch_id, a.type, a.status, a.car_no as dispatch_car_no, c.driver, c.mobile, d.id_no, a.trans_status, a.add_time as op_time, b.* from dispatch as a left join container as b on b.id = a.container_id left join vehicle_info as c on c.car_no = a.car_no left join driver_info as d on d.mobile = c.mobile where a.type = '拆箱' and a.status = '已派车' ";
